 ...
 def onEvent (self,event,st) :
   ...
   if event == "31272-5.htm" :
     if cond == 20 and st.getPlayer().isSubClassActive() :
       if st.getQuestItemsCount(LUNARGENT) < 5 or not st.getQuestItemsCount(HELLFIRE_OIL) :
 ...
 def onTalk (self,npc,player):
   ...
   if player.isSubClassActive() :
     if npcId == TALIEN :
       ...
       elif cond == 20 and (st.getQuestItemsCount(LUNARGENT) < 5 or not st.getQuestItemsCount(HELLFIRE_OIL)) :
         htmltext = "31272-4.htm"
